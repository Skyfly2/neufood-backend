...
describe("PUT /date/:ingredients_id", () => {
  let connection;
  let db;
  let mongoServer;
  const ingredient = {
  ...
  it("should return 200 if the request is added successfully", async () => {
    ...
    const res = await request(app).put("/date/ingredients/123456789123");
    expect(res.status).toBe(200);
  });

  it("should return 400 if the ingredient ID is invalid", async () => {
    jest.spyOn(MongoClient, "connect").mockReturnValue(connection);
    const res = await request(app).put("/date/ingredients/456789123456");
